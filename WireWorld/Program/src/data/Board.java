...
public class Board {
    ...
    public int readBoardFromFile(String path) throws IOException { //nale�y doda� obs�ug� wszystkich element�w opr�cz diody
    ...
    public int fillWithElement(Element e) {
        ...
        for (Point p : elementLocation) {
            ...
            if (p.x >= 0 && p.x < HEIGHT & p.y >= 0 && p.y < WIDTH) {
                cells[p.y][p.x] = Element.CONDUCTOR; // celowe wstawianie w ten spos�b, �eby nie "odwr�ci�" uk�adu
                // wsp�rz�dnych
            } else {
            ...
            for (int j = 0; j < WIDTH; j++) {
                switch (cells[i][j]) {
                    ...
                    case Element.ELECTRON_TAIL:
                        ...
                        bufferedWriter.newLine();
                        break;
                    /*case Element.CONDUCTOR: //moim zdaniem bez sensu(duplikacja tego co napisaliśmy wcześniej + wypisywanie pustych komórek)
